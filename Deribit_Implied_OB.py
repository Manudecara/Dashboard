...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 13 16:17:56 2021
...
if __name__ == '__main__':
    ...
    all_days.append(calendar.timegm(present.timetuple()))

    for i in list(range(1,4)): #adjust for number of days
        past = present - dt.timedelta(days=i)
        all_days.append(calendar.timegm(past.timetuple()))
    
    start = list(range(0,len(all_days)))
    end = list(range(1,len(all_days)))
    start = start[:-1]
    ...
    for s,e in zip(start, end):
        for i in options.instrument_name:
            markets.append(ws.get_last_trades_by_instrument_and_time(instrument_name = i, 
                                                                     start_timestamp = all_days[e], 
                                                                     end_timestamp = all_days[s], 
                                                                     count = 1000,
                                                                     sorting = 'desc'))    
    
    #len(markets)/len(options) 
    #check you have right number of days

    day1 = markets[0: len(options)]
    day2 = markets[len(options):len(options)*2]
    day3 = markets[len(options)*2:len(markets)]
    
    day1 = [i for i in day1 if i != []]
    day2 = [i for i in day2 if i != []]
    day3 = [i for i in day3 if i != []]
    
    days = [day1, day2, day3]

    #last_trades = [i for i in last_trades if len(i) > 10]
    ...
    def DDOI():
        ...
        for day in days:
            for market in day:
               for trade in market:
                    ddoi.append(trade.get('instrument_name').rpartition('-')[2])                                        #C or Pffffffffffffffffffffffffffff
    ...
    def GEX():            
        ...
        flag = list(range(0, len(ddoi), 6))    
        underlying = list(range(1, len(ddoi), 6))    
        strike = list(range(2, len(ddoi), 6))    
        ttm = list(range(3, len(ddoi), 6))    
        rfr = list(range(4, len(ddoi), 6))    
        vol = list(range(5, len(ddoi), 6))
        
        deltas = []
        for f,u,s,t,r,v in zip(flag, underlying, strike, ttm, rfr, vol):
            deltas.append(delta(ddoi[f],  ddoi[u],  ddoi[s],  ddoi[t],  ddoi[r],  ddoi[v]))
        
        delta1 = deltas[0:len(day1)]
        delta2 = deltas[len(day1):(len(day1)+len(day2))]
        delta3 = deltas[(len(day1)+len(day2)):(len(day1)+len(day2)+len(day3))]
...
'''
df_last_trades = pd.DataFrame(last_trades)
    
    empty_cells = []
    for v,i in zip(df_last_trades[0], df_last_trades.index):
        if v == None:
